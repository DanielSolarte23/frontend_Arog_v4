  ...
  return (
    <div className="w-full bg-white/10 filter border rounded-lg shadow-xl border-t px-4 sm:px-8 md:px-14 py-6 sm:py-8 md:py-12 flex flex-col justify-evenly">
      {RegisterErrors.map((error, i) => (
        <div
          className={`${
            error.includes("verificación") ? "text-green-500" : "text-red-500"
          }`}
          key={i}
        >
          {error}
      ...
      <h2 className="text-2xl sm:text-3xl font-bold text-verde-dos">
        Registro
      </h2>
      ...
      <form onSubmit={onSubmit} className="">
          ...
          <div className="mt-2">
            <div className="w-full h-12 md:h-14 relative">
              <input
                ...
                className={`block h-full w-full px-4 py-2 text-verde-dos placeholder:text-gray-600 placeholder:font-thin bg-transparent border-b-4 border-verde-dos hover:border-verde rounded-lg focus:outline-none text-base sm:text-lg md:text-xl pl-10 focus:border-verde ${
                  errors.nombres ? "border-red-500" : "border-verde"
                }`}
              />
          ...
          <div className="mt-2">
            <div className="w-full h-12 md:h-14 relative">
              <input
                ...
                className={`block h-full w-full px-4 py-2 text-verde-dos placeholder:text-gray-600 placeholder:font-thin bg-transparent border-b-4 border-verde-dos hover:border-verde focus:border-verde rounded-lg focus:outline-none text-base sm:text-lg md:text-xl pl-10 ${
                  errors.apellidos ? "border-red-500" : "border-verde"
                }`}
              />
          ...
          <div className="mt-2">
            <div className="w-full h-12 md:h-14 relative">
              <input
                ...
                className={`block h-full w-full px-4 py-2 text-verde-dos placeholder:text-gray-600 placeholder:font-thin bg-transparent border-b-4 border-verde-dos hover:border-verde focus:border-verde rounded-lg focus:outline-none text-base sm:text-lg md:text-xl pl-10 ${
                  errors.telefono ? "border-red-500" : "border-verde"
                }`}
              />
        ...
        {showRequirements && (
          <div className="w-full mt-3 md:mt-5 px-3 md:px-4 py-2 bg-yellow-100 border border-yellow-300 flex items-center gap-2 rounded-md">
            ...
            <p className="text-yellow-700 text-xs md:text-sm">
              La contraseña debe tener más de 8 caracteres, al menos una letra
        ...
        <button
          type="submit"
          disabled={loading}
          className={`w-full px-4 md:px-6 py-2 h-12 md:h-14 text-sm md:text-lg font-medium tracking-wide text-white capitalize transition-colors duration-300 transform bg-verde-principal rounded-lg hover:bg-lime-600 focus:outline-none focus:ring focus:ring-gray-300 focus:ring-opacity-50 ${
            showRequirements ? "mt-3" : "mt-4 sm:mt-6 md:mt-8"
      ...
      <p className="mt-3 text-xs sm:text-sm md:text-base font-light text-center text-white">
        ...
        <Link href="/auth/inicio" className="text-verde-dos font-semibold">
          Inicia sesión
        </Link>
      </p>
    </div>
