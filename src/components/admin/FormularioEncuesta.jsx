...
export default function EncuestaForm({ encuesta = null, onBack, onSuccess }) {
    ...
    useEffect(() => {
        if (encuesta) {
            ...
            formattedEncuesta.preguntas = Array.isArray(encuesta.preguntas) ? encuesta.preguntas : [];
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const dataToSend = {
                ...formData,
                creadorId: 1, // Esto deberías obtenerlo de tu sistema de autenticación
            };
    ...
    const handleQuestionChange = (index, field, value) => {
        setFormData(prev => {
            ...
            if (field === 'tipo' && value !== 'texto_libre' && (!updatedPreguntas[index].opciones || updatedPreguntas[index].opciones.length === 0)) {
                updatedPreguntas[index].opciones = [
                    { texto: 'Opción 1' },
                    { texto: 'Opción 2' }
                ];
    ...
    const addOption = (questionIndex) => {
        setFormData(prev => {
            ...
            updatedPreguntas[questionIndex].opciones = [
                ...(updatedPreguntas[questionIndex].opciones || []),
                { texto: `Opción ${(updatedPreguntas[questionIndex].opciones?.length || 0) + 1}` }
            ];
    ...
    const removeOption = (questionIndex, optionIndex) => {
        setFormData(prev => {
            const updatedPreguntas = [...prev.preguntas];
            updatedPreguntas[questionIndex].opciones = updatedPreguntas[questionIndex].opciones.filter((_, i) => i !== optionIndex);
            ...
            <div className="p-6">
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="grid grid-cols-2 gap-6 mb-6">
                        <div>
                            <label className="block text-gray-700 text-sm font-bold mb-2" htmlFor="fechaExpiracion">
                                Fecha de expiración
                            </label>
                            <input
                                id="fechaExpiracion"
                    ...
                    <div className="border-t border-gray-200 pt-6 mb-6">
                        ...
                        <div className="space-y-4">
                            {formData.preguntas.map((pregunta, questionIndex) => (
                                <div key={questionIndex} className="border border-gray-200 rounded">
                                    <div
                                            ...
                                            {(pregunta.tipo === 'opcion_multiple' || pregunta.tipo === 'seleccion_unica') && (
                                                <div className="mb-4">
                                                    ...
                                                    {(pregunta.opciones || []).length < 2 && (
                                                        <p className="text-red-500 text-xs mt-1">Se requieren al menos 2 opciones</p>
                                                    )}
                                                </div>
                                            )}
                                            ...
                                            {pregunta.tipo === 'escala' && (
                                                <div className="mb-4">
                                                    ...
                                                    <p className="text-sm text-gray-600 mb-2">
                                                        La escala será del 1 al 5, donde 1 es el valor mínimo y 5 es el valor máximo.
                                                    </p>
                                                </div>
                                            )}
                                        </div>
