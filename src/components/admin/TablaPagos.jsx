...
export default function PagosTabla({ isPago, setisPago }) {
    // const { pagos, getPagos, updatePago, deletePago, getPago, createPago, error, loading } =
    //     usePago()
    ...
    //         const nombreCliente = pago.cliente && pago.cliente.nombre ? pago.cliente.nombre.toLowerCase() : '';
    //         const apellidoCliente = pago.cliente && pago.cliente.apellido ? pago.cliente.apellido.toLowerCase() : '';
        ...
        <div className="relative overflow-x-auto bg-white h-full border-gray-200 rounded-lg">
            ...
            <nav className="bg-white border-b border-b-gray-200 flex flex-col md:flex-row items-center justify-between py-2 px-4 gap-4 h-[15%] xl-plus:h-1/10">
                ...
                <div className="flex items-center gap-2 md:w-auto">
                    <button className={` border p-2 rounded-lg text-verde-dos flex items-center gap-2  transition w-full md:w-auto justify-center px-4 ${isPago ? "bg-white hover:bg-lime-50" : "bg-lime-600 text-white"}`}
                        onClick={() => setisPago(false)}>
                        Pagos
                    </button>
                    <button className={`border p-2 rounded-lg text-verde-dos flex items-center gap-2 transition w-full md:w-auto justify-center px-4 ${!isPago ? "bg-white-600 text-verde-dos hover:bg-lime-100" : "bg-verde text-white"}`}
                    ...
                    >
                        <span className="font-medium">{isPago ? "Registrar cliente" : "Registrar pago"}</span>
                        <svg
                    ...
                    <table className="text-sm text-left text-gray-500 w-full">
                        ...
                        <thead className="text-xs text-gray-700 uppercase bg-white border-b border-gray-200">
                            <tr>
                                ...
                                <th className="px-4 py-3 md:px-6 md:py-4">Escripcion</th>
                                <th className="px-4 py-3 md:px-6 md:py-4">Fecha Pago</th>
                                <th className="px-4 py-3 md:px-6 md:py-4">Vencimiento</th>
                                <th className="px-4 py-3 md:px-6 md:py-4">Estado Mora</th>
                                <th className="px-4 py-3 md:px-6 md:py-4">Dias Mora</th>
                                <th className="px-4 py-3 md:px-6 md:py-4">Detalles</th>
                            </tr>
                        </thead>
                                ...
                                >
                                    ...
                                    <td className="px-4 py-2 md:px-6 md:py-4">
                                        {new Intl.DateTimeFormat("es-CO", {
                                            dateStyle: "medium",
                                        }).format(new Date(pago.fechaPago))}
                                    </td>
                                    <td className="px-4 py-2 md:px-6 md:py-4">
                                        {new Intl.DateTimeFormat("es-CO", {
                                            dateStyle: "medium",
                                    ...
                                    <td className="px-4 py-2 md:px-6 md:py-4">{pago.estadoMora === true ? (<span className="bg-red-200 p-1 rounded-lg">En mora</span>) : (<span className="bg-green-200 p-1 rounded-lg">Al dia</span>)}</td>
                                    <td className="px-4 py-2 md:px-6 md:py-4">{pago.diasMora || ""}</td>
            ...
            {modalOpen && (
                <FormularioPago handleCloseModal={handleCloseModal} pagoSeleccionado={pagoSeleccionado} handleSubmit={handleSubmit} handleInputChange={handleInputChange} nuevoPago={nuevoPago} />
            )} */}
        </div>
    );
}