...
export default function ClientesTabla({ isPago, setisPago }) {
    const { clientes, getClientes, updateCliente, deleteCliente, getcliente, createCliente, error, loading } = useCliente();
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (resultado && (resultado.success || resultado.status === 200 || resultado.status === 201)) {
                ...
                setTienePlan(false);
                showNotification(clienteSeleccionado ? "Cliente actualizado exitosamente" : "Cliente registrado exitosamente");
        ...
        <div className="relative overflow-x-auto bg-white h-full border-gray-200 rounded-lg">
            ...
            <nav className="bg-white border-b border-b-gray-200 flex flex-col md:flex-row items-center justify-between py-2 px-4 gap-4 h-[15%] xl-plus:h-1/10">
                ...
                <div className="flex items-center gap-2 md:w-auto">
                    ...
                    >
                        <span className="font-medium">{isPago ? "Registrar cliente" : "Registrar cliente"}</span>
                        <svg
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {currentItems.map((cliente) => (
                                ...
                                >
                                    ...
                                    <td className="px-4 py-2 md:px-6 md:py-4">{cliente.telefono || ""}</td>
                                    <td className="px-4 py-2 md:px-6 md:py-4">{cliente.direccion || ""}</td>
                                    <td className="px-4 py-2 md:px-6 md:py-4">{cliente.pagos?.fechaVencimiento || "sin pagos pendientes"}</td>
