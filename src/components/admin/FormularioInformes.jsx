...
export default function FormularioInforme({ cerrarFormulario, getDocumentoInforme }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await postDocumentos(formData, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      });

      // Capturar el código generado por el backend para mostrarlo
      if (response.data && response.data.codigoReferencia) {
        setDatos({ ...datos, codigoInforme: response.data.codigoReferencia });
      }
