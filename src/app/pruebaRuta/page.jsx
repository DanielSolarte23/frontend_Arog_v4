...
export default function FormularioInforme() {
  ...
  const generarPDFTecnico = () => {
    ...
    doc.text('INFORME TÉCNICO', doc.internal.pageSize.getWidth() / 2, yPos, { align: 'center' });
    yPos += lineHeight * 2;
    ...
    doc.text(`FECHA: ${new Date(datos.fechaEmision).toLocaleDateString('es-PE')}`, 20, yPos);
    yPos += lineHeight * 2;
  ...
  const generarPDFGestion = () => {
    ...
    doc.setFontSize(16);
    doc.setFont('helvetica', 'bold');
    doc.text('INFORME DE GESTIÓN', doc.internal.pageSize.getWidth() / 2, yPos, { align: 'center' });
    yPos += lineHeight * 2;
  ...
  const generarPDFEvaluacion = () => {
    ...
    doc.setFontSize(16);
    doc.setFont('helvetica', 'bold');
    doc.text('INFORME DE EVALUACIÓN', doc.internal.pageSize.getWidth() / 2, yPos, { align: 'center' });
    yPos += lineHeight * 2;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formData.append('file', pdfBlob, `informe_${datos.tipoInforme.toLowerCase()}_${datos.codigoInforme}.pdf`);
  ...
  return (
    <div className="container mx-auto p-4">
      <h2 className="text-2xl font-bold mb-4">Generador de Informes</h2>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4 mb-4">
        ...
        <div className="mt-4">
          <label className="block text-sm font-medium mb-1">Resumen/Contenido principal</label>
          <textarea
        ...
        <div className="mt-2">
          <label className="block text-sm font-medium mb-1">Conclusiones/Recomendaciones</label>
          <textarea
