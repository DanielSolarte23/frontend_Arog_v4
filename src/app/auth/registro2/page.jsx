...
import { useAuth } from "@/context/AauthContext";
...
function RegistesPage() {
  ...
  const onSubmit = handleSubmit(async (values) => {
    signup(values);
    // No redirigir automáticamente - esperar verificación
  });
          ...
          <div className="border 2xl:py-5 xl:py-3 px-10 rounded-xl  h-[90%] contain shadow-2xl">
            ...
            <div className="h-[10%]">
              {RegisterErrors.map((error, i) => (
                <div className={`${error.includes("verificación") ? "text-green-500" : "text-red-500"
                  }`} key={i}>
                  {error}
                </div>
              ))}
